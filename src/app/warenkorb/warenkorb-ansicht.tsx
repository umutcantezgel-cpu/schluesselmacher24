...
function zylinderZeilen(
  ...
  const formMap = new Map(catalog.forms.map((f) => [f.id, f]));
  const funcMap = new Map(catalog.functions.map((f) => [f.id, f]));
  const extraMap = new Map(catalog.extras.map((e) => [e.id, e]));

  for (const eintrag of draft.items) {
    const form = formMap.get(eintrag.form);
    const funktion = eintrag.functionId ? funcMap.get(eintrag.functionId) : undefined;
    const masse =
